...
const socketURL = ref(__APP_CONFIG__.socket);
const apiURL = ref(__APP_CONFIG__.apiURL);
...
const host = computed(() => {
    ...
    return data.value.filter(
        (item) => item.Host.Name.slice(0, 2) === selectArea.value,
    );
});
...
const initScoket = async () => {
    ...
    socket.onmessage = function (event) {
        try {
            ...
            if (res != null) {
                area.value = Array.from(
                    new Set(res.map((item) => item.Host.Name.slice(0, 2))),
                );
            }
            data.value = res.map((host) => {
                ...
                if (charts.value[host.Host.Name].cpu.length == 2) {
                    charts.value[host.Host.Name].cpu =
                        charts.value[host.Host.Name].cpu.slice(1);
                    charts.value[host.Host.Name].mem =
                        charts.value[host.Host.Name].mem.slice(1);
                    charts.value[host.Host.Name].net_in =
                        charts.value[host.Host.Name].net_in.slice(1);
                    charts.value[host.Host.Name].net_out =
                        charts.value[host.Host.Name].net_out.slice(1);
                }

                charts.value[host.Host.Name].cpu.push([
                    host.TimeStamp * 1000,
                    host.State.CPU,
                ]);
                charts.value[host.Host.Name].mem.push([
                    host.TimeStamp * 1000,
                    host.State.MemUsed,
                ]);
                charts.value[host.Host.Name].net_in.push([
                    host.TimeStamp * 1000,
                    host.State.NetOutSpeed,
                ]);
                charts.value[host.Host.Name].net_out.push([
                    host.TimeStamp * 1000,
                    host.State.NetInSpeed,
                ]);
...
<template>
    <div class="max-container">
        ...
        <div class="monitor-card">
            ...
            >
                <div class="name">
                    ...
                    >
                        <span>{{
                            item.status ? $t("online") : $t("offline")
                        }}</span>
                        <span style="margin-left: 6px">{{
                            formatUptime(item.State.Uptime)
                        }}</span>
                    </div>
                </div>
                <div class="platform">
                ...
                <div class="mem">
                    ...
                    <div class="monitor-item-value">
                        {{
                            (
                                (item.State.MemUsed / item.Host.MemTotal) *
                                100
                            ).toFixed(2) + "%"
                        }}
                    </div>
                    <a-progress
                        ...
                        :status="
                            progressStatus(
                                (item.State.MemUsed / item.Host.MemTotal) * 100,
                            )
                        "
                    ...
                    <div class="monitor-item-value">
                        {{
                            hostInfo[item.Host.Name]
                                ? calculateRemainingDays(
                                      hostInfo[item.Host.Name].due_time,
                                  )
                                : "-"
                        }}
                    </div>
                            ...
                            <div class="detail-item-list">
                                ...
                                <div class="detail-item">
                                    ...
                                    <div class="value">
                                        {{
                                            formatBytes(
                                                item.State.NetOutTransfer,
                                            )
                                        }}
                                        |
                                        {{
                                            formatBytes(
                                                item.State.NetInTransfer,
                                            )
                                        }}
                                    </div>
                                </div>
                                <div class="detail-item">
                                    ...
                                    <div class="value">
                                        {{
                                            formatTimeStamp(item.Host.BootTime)
                                        }}
                                    </div>
                                </div>
                                <div class="detail-item">
                                ...
                                <div
                                    ...
                                    v-if="
                                        hostInfo[item.Host.Name] &&
                                        hostInfo[item.Host.Name].seller
                                    "
                                >
                        ...
                        >
                            <a-row :gutter="20">
                                ...
                                >
                                    <NetIn
                                        ref="netInRef"
                                        :data="charts[item.Host.Name].net_in"
                                    />
                                </a-col>
                                <a-col
                                    :span="12"
                                ...
                                >
                                    <NetOut
                                        ref="netOutRef"
                                        :data="charts[item.Host.Name].net_out"
                                    />
                                </a-col>
                            </a-row>
                        </a-col>
        ...
        <div class="footer" style="margin-top: 30px">
            {{ $t("open-source") }}
            <a href="https://github.com/akile-network/akile_monitor"
                >GitHub v0.0.3</a
            >
        </div>
        <div class="footer" style="margin-bottom: 30px">
            Copyright © 2023-{{ new Date().getFullYear() }} Akile LTD.
        </div>
    </div>
</template>
